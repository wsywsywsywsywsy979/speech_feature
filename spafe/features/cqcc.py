...
def cqt_spectrogram(
    ...
    # -> compute constant Q-transform 对应CQT
...
def cqcc(
    ...
    constant_qtransform = cqt_spectrogram(
        ...
        fs=fs,
        pre_emph=pre_emph, # 1
        pre_emph_coeff=pre_emph_coeff,# 0.97
        win_len=win_len,# 0.02
    ...
    # -> log(.)
    # handle zeros: if feat is zero, we get problems with log # 对应对数功率
    features_no_zero = zero_handling(x=power_spectrum)
    log_features = np.log(features_no_zero)

    # uniform resampling # 对应均匀采样
